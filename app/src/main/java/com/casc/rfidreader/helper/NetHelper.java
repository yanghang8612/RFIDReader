...
public class NetHelper implements Callback<Reply> {
    ...
    public Call<Reply> userLogin(String username, String password) {
        return netInterface.userLogin(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/login",
                username,
                password);
    }
    ...
    public Call<Reply> sendHeartbeat() {
        return netInterface.sendHeartbeat(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/heartbeat");
    }

    public Call<Reply> uploadTask(MessageUpload message) {
        return netInterface.uploadTask(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/debugdata",
                CommonUtils.generateRequestBody(message));
    }

    public Call<Reply> queryTask(long starttime, long endtime) {
        Map<String, String> map = new HashMap<>();
        map.put("username", ConfigHelper.getParam(MyParams.S_USERNAME));
        map.put("starttime", String.valueOf(starttime));
        map.put("endtime", String.valueOf(endtime));
        return netInterface.queryTask(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/outlinequery",
                map);
    }

    public Call<Reply> queryDetail(String taskid) {
        return netInterface.queryDetail(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/detailedquery",
                ConfigHelper.getParam(MyParams.S_USERNAME),
                taskid);
    }

    public Call<Reply> requestTaskID() {
        String username = ConfigHelper.getParam(MyParams.S_USERNAME);
        return netInterface.requestTaskID(
                ConfigHelper.getParam(MyParams.S_MAIN_PLATFORM_ADDR) + "/api/debugtool/taskid",
                ConfigHelper.getParam(MyParams.S_USERNAME));
    }
